...
func main() {
	...
	if len(result["body"].(map[string]interface{})["songs"].([]interface{})) > 0 {
		for i := 0; i < len(result["body"].(map[string]interface{})["songs"].([]interface{})); i++ {
			...
			album := ParseAlbum(id, i, result)
			fmt.Println(artistMap)
			fmt.Println(name, artist, album)
			filename := fileName[i]
			MusicMarker(id, filename, name, i, options, result, artistMap)
...
func MusicMarker(id, filename, name string, s int, options, result, artistMap map[string]interface{}) (marker string) {
	var data map[string]interface{}
	data = make(map[string]interface{})
	format := path.Ext(path.Base(options["savePath"].(string) + filename))
...
func ParseAlbum(id string, i int, result map[string]interface{}) (album string) {
